...
class WebSocketServer: protected Server{
        protected:
        // Server tcp;
		...
		*payload_length = frame_buffer [1] & 0x7F;
        ...
        int webSocketCreate (){
                char buffer [2048];
                int client_socket = connection_accepting ();
                if (client_socket == -1)
                return -1;

                int len = recvResponse (client_socket, buffer, 2048);
        ...
        int send_websocket_frame (int client_socket, uint8_t fin, uint8_t opcode, char *payload) {
                ...
                ssize_t bytes_sent = sendRequest (client_socket, encoded_data, encoded_size);
        ...
        void sendCloseFrame (int client_socket){
                ...
                sendRequest (client_socket, close_frame, sizeof (close_frame));
        ...
        int recv_websocket_frame (char **decodedData, int client_socket){
                ...
                if ((len = recvResponse (client_socket, data, length)) == -1)
                        return -1;
...
class ChatServer: protected WebSocketServer{
        public:
        ...
        int addClient(){
                Client new_client;
                new_client.setSockFd(webSocketCreate());
		...
		if(flag){
			send_websocket_frame(sockfd, 1, 1, response);
			char success[14] = "message sent.";
			send_websocket_frame(sender_sockfd, 1, 1, success);
		} else {
			char message[17] = "user not found!!";
			send_websocket_frame(sockfd, 1, 1, message);
		}
	}

        void broadcastMessage(char *response, int sender_sockfd){
                for(auto i: clients){
                        send_websocket_frame(i.first, 1, 1, response);
                }
        }
        ...
        void *handleClient(int sockfd){
                ...
                int flag = recv_websocket_frame(&decoded_name, sockfd);
                ...
                while(1){
                        ...
                        if ((flag = recv_websocket_frame(&decoded_data, sockfd)) == -1)
                                break;
